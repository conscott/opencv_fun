...
cv::Rect resizeRoi(const cv::Rect & roi, const double & scale, const int & max_row, const int & max_col) {
    ...
    // Resize rectangle by scale
    int nwidth= int(double(roi.width) * scale);
    int nheight = int(double(roi.height) * scale); 
...
// Use the CascadeClassifier to detect faces in a single images, returning
// a vector of rectangles containing the face locations
...
void detectAndDraw( cv::Mat& img, cv::CascadeClassifier& cascade, FaceTracker& faceTracker, const int& nFrame)
{
    ...
    // Make incoming image RBGA, need to work with alpha channel to overlay masks
    cv::Mat imga;
    ...
    // Submit face detections to FaceTracker, which ill 
    // incorporate it to the tracker faces if it has met the 
    // detection threshholds
    faceTracker.processNewDetections(facesInFrame, nFrame);

    // Now get the list of faces we think are real
    auto& facesReal = faceTracker.getFaces();

    for(auto& face : facesReal) {

        // Each face gets disguised by a mask that we blend with the webcam image
